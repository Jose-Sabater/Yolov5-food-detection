...
def run(
    weights=ROOT / "bestmedium.pt",  # model.pt path(s)
    source=ROOT / "./images/image.jpg",  # file/dir/URL/glob, 0 for webcam
    ...
    project=ROOT / "static/results",  # save results to project/name
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # Write results
                results_to_return = reversed(det)  ##added by Jose
    ...
    if save_txt or save_img:
        ...
        return results_to_return  # added by jose
    if update:
        strip_optimizer(
            weights[0]
...
def main(source, name):
    ...
    results = run(source=source, name=name)
    return results
